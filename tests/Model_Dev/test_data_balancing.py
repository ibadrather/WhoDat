...
def test_balance_dataframe_by_resampling_class():
    ...
    assert (
        balanced_df["class"].value_counts().iloc[0] == 100
    ), "Oversampling failed: incorrect count for class 0"
    assert (
        balanced_df["class"].value_counts().iloc[1] == 100
    ), "Oversampling failed: incorrect count for class 1"
    ...
    assert (
        balanced_df["class"].value_counts().iloc[0] == 10
    ), "Undersampling failed: incorrect count for class 0"
    assert (
        balanced_df["class"].value_counts().iloc[1] == 10
    ), "Undersampling failed: incorrect count for class 1"
