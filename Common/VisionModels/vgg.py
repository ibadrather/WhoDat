"""
    Source: https://github.com/Lornatang/VGG-PyTorch/blob/main/model.py
"""
...
def _make_layers(
    vgg_cfg: List[Union[str, int]], batch_norm: bool = False, input_channels: int = 3
) -> nn.Sequential:
    ...
    for v in vgg_cfg:
        if v == "M":
            ...
            v = cast(int, v)
            conv2d = nn.Conv2d(input_channels, v, (3, 3), (1, 1), (1, 1))
            if batch_norm:
...
class VGG(nn.Module):
    def __init__(
        ...
        self.features = _make_layers(vgg_cfg, batch_norm, input_channels=input_channels)
