"""
    Source : https://github.com/Lornatang/GoogLeNet-PyTorch/blob/main/model.py
"""
...
class GoogLeNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = BasicConv2d(
            input_channels, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3)
        )
        self.maxpool1 = nn.MaxPool2d((3, 3), (2, 2), ceil_mode=True)
        self.conv2 = BasicConv2d(
            64, 64, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0)
        )
    ...
    ) -> GoogLeNetOutputs | Tensor:
        if self.training and self.aux_logits:
            return GoogLeNetOutputs(x, aux2, aux1)._asdict()["logits"]
        else:
            return x
    ...
    def _forward_impl(self, x: Tensor) -> GoogLeNetOutputs:
        ...
        if torch.jit.is_scripting():
            return GoogLeNetOutputs(aux3, aux2, aux1)._asdict()["logits"]
        else:
            return self.eager_outputs(aux3, aux2, aux1)  # ._asdict()["logits"]

    def _initialize_weights(self) -> None:
        for module in self.modules():
            if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
                torch.nn.init.trunc_normal_(
                    module.weight, mean=0.0, std=0.01, a=-2, b=2
                )
...
class Inception(nn.Module):
    def __init__(
        ...
        self.branch1 = BasicConv2d(
            in_channels, ch1x1, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0)
        )

        self.branch2 = nn.Sequential(
            BasicConv2d(
                in_channels, ch3x3red, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0)
            ),
            BasicConv2d(
                ch3x3red, ch3x3, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)
            ),
        )

        self.branch3 = nn.Sequential(
            BasicConv2d(
                in_channels, ch5x5red, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0)
            ),
            BasicConv2d(
                ch5x5red, ch5x5, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)
            ),
        )

        self.branch4 = nn.Sequential(
            nn.MaxPool2d(
                kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), ceil_mode=True
            ),
            BasicConv2d(
                in_channels,
                pool_proj,
...
class InceptionAux(nn.Module):
    def __init__(
        ...
        self.conv = BasicConv2d(
            in_channels, 128, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0)
        )
        self.relu = nn.ReLU(True)
